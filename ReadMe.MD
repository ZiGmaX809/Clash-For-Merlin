...
# redirect for clash except Nintendo Switch and udp
iptables -t mangle -I PREROUTING -p udp -j RETURN
iptables -t nat -I PREROUTING -p udp -j RETURN
 ...
 * #### 网络环境中游戏问题及无需代理的设备
   ...
   暂行解决思路：
   	1、在iptable中排除除了请求DNS的udp数据，仅对tcp数据进行处理；
	2、使用redir-host；
