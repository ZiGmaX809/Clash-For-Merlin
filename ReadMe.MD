...
# except udp
iptables -t nat -A PREROUTING -p udp -j RETURN
```
 ...
 * #### 网络环境中游戏问题
   fake-ip模式下，由于所有流量都会经过Clash，所以无法合理区分诸如王者荣耀等游戏的udp数据，从而会导致游戏丢包以及掉线；
   暂行解决思路：
   	1、在iptable中排除除了请求DNS的udp数据；
	2、使用redir-host；
